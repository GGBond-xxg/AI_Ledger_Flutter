...
class _AssetFormPageState extends State<AssetFormPage> {
  ...
  bool get _needsSymbol => ['crypto', 'metal', 'stock', 'etf'].contains(_type);
  bool get _needsMarketSuggestions => ['crypto', 'metal', 'stock', 'etf'].contains(_type);
  ...
  void _scheduleRemoteSearch({bool immediate = false}) {
    ...
    final canSearchRemote = store.settings.apiBaseUrl.trim().isNotEmpty &&
        ...
        ['stock', 'etf', 'crypto'].contains(_type);
    ...
    _searchDebounce = Timer(immediate ? Duration.zero : const Duration(milliseconds: 450), () async {
      ...
      _refreshUi();
      try {
        final items = await store.searchMarket(_type, query, limit: 12);
        if (!mounted) return;
  ...
  List<MarketOption> _marketSuggestions() {
    ...
    return mergeMarketSuggestions(marketSuggestionsFor(_type, _currentQuery(), limit: 10), _remoteSuggestions, limit: 12);
  }
