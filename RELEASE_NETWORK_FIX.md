...
iOS 使用 `https://api.example.com` 这种 HTTPS 地址不需要额外网络权限。相册/相机权限保留在 `Info.plist` 即可。

如果 iOS 也请求失败，优先检查：

1. API 地址是否是 `https://api.example.com`，不要填错域名。
2. 手机是否能用 Safari 打开 `https://api.example.com/api/health?token=你的token`。
